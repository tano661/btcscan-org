...
const formatTime = (unix, with_tz = true) => {
...
export default function main({ DOM, HTTP, route, storage, scanner: scan$, search: searchResult$, blinding: unblinded$ }) {
  const
    ...
    , description$ = O.merge(page$.withLatestFrom(t$, (_, t) => t`Btcscan's Bitcoin block explorer provides real-time blockchain data. With the BTC block explorer, you can search blocks, addresses, and transactions on the Bitcoin blockchain`)
      ...
      , addr$.filter(notNully).withLatestFrom(goAddr$, t$, (_, goAddr, t) => t`Check Bitcoin (BTC) Address ${goAddr.display_addr} current balance and its transactions history with BTCScan`))

    , ogTitle$ = O.merge(page$.withLatestFrom(t$, (_, t) => t`Btcscan's Bitcoin block explorer provides real-time blockchain data. With the BTC block explorer, you can search blocks, addresses, and transactions on the Bitcoin blockchain`)
      , goBlocks$.withLatestFrom(t$, (_, t) => t`Btcscan’s Bitcoin blockchain explorer helps make sense of past and real-time network data. Search BTC explorer for information on blocks, transactions, and addresses`)
      , goRecent$.withLatestFrom(t$, (_, t) => t`Check bitcoin transactions using Btcscan. Btcscan is a tool for tracking BTC transactions. ID of the transaction, bitcoin value, size, and fee can all be found using the bitcoin block explorer`)
      , block$.filter(notNully).withLatestFrom(t$, (block, t) => t`Bitcoin block ${block.height}`)
      , tx$.filter(notNully).withLatestFrom(t$, (tx, t) => t`Bitcoin Transaction ${tx.txid}` )
      , addr$.filter(notNully).withLatestFrom(goAddr$, t$, (_, goAddr, t) => t`Bitcoin Address ${goAddr.display_addr}`))
    ...
    , ogUrl$ = O.merge(page$.withLatestFrom(t$, (_, t) => t`Btcscan's Bitcoin block explorer provides real-time blockchain data. With the BTC block explorer, you can search blocks, addresses, and transactions on the Bitcoin blockchain`)
      , goBlocks$.withLatestFrom(t$, (_, t) => t`Btcscan’s Bitcoin blockchain explorer helps make sense of past and real-time network data. Search BTC explorer for information on blocks, transactions, and addresses`)
      , goRecent$.withLatestFrom(t$, (_, t) => t`Check bitcoin transactions using Btcscan. Btcscan is a tool for tracking BTC transactions. ID of the transaction, bitcoin value, size, and fee can all be found using the bitcoin block explorer`)
